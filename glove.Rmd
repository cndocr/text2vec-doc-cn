...
1. 使用词共现矩阵收集词共现信息。每个 $X_{ij}$ 元素代表词汇 *i* 出现在词汇 *j*上下文的概率。一般我们需要扫描一遍我们的语料库：对于每一个字段，我们查看在这个字段之前或者之后，使用 *window_size* 定义的一定范围内的上下文。一个词距离这个字段越远，我们给予这个词的权重越低。一般使用这个公式： $$decay = 1/offset$$
